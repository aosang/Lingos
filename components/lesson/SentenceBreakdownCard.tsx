import { Dimensions, Pressable, StyleSheet, View, ScrollView } from "react-native";
...
export default function SentenceBreakdownCard ({
  ...
  const showTooltip = (word: Word, type: "hanzi" | "pinyin", index: number) => {
    const wordRef = type === "hanzi"? hanziWordRefs.current[index] : pinyinWordRefs.current[index]
    if(!wordRef) return
        ...
        {sentence.words.map((word, index) => (
          ...
          >
            <ThemedText style={[
              ...
              selectedWord && selectedWord.type === type && selectedWord.index === index && styles.selectedWord
            ]}>
              {word[type]}{" "}
            </ThemedText>
          </Pressable>
