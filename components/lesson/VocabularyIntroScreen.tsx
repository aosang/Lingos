...
export default function VocabularyIntroScreen ({
  ...
  const [state, setState] = useState<StudyState>(() => initializeStudyState(deck))
  ...
  const headerCount = currentCard?  Math.min(state.completed + 1, state.total) : state.completed
